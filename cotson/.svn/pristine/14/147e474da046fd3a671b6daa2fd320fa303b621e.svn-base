...
class GraphicData
  ...
  def initialize(eid)
    ...
    @experiment = Experiment.find(eid)
  ...
  def push_series_data(data)
    ...
    puts "data_transform duration: #{@time[:transform]} sec"
...
#        puts "   --- ROW: nanos=#{nanos} -- value=#{row.value}"
    ...
    xml.chart do
      ...
      xml.graphs do
        data.each do |serie, values|
          xml.graph :gid => "#{serie}" do
            values.each_with_index do |nanos,i|
                xml.value nanos, :xid => i, :description => "#{serie[0..15]} [#{nanos} ms]"
            end
          end
        end
      end
  ...
  def cache_data
    ...
    $data_cache[@experiment.experiment_id][@preset_metric] = @series_data
  end
  ...
  def recover_from_cache
    ...
    @series_data = $data_cache[@experiment.experiment_id][@preset_metric]
