...
// $Id: test_moesi_2levels.cpp 721 2015-06-03 02:29:33Z paolofrb $

#define BOOST_TEST_MODULE test_moesi_2levels
...
BOOST_AUTO_TEST_CASE( test_moesi_states_with_bus_2levels_WB_ )
{
	...
	//from MODIFIED to OWNER in L1_A )
	...
	time+=lat+TIME_TO_STABLE;
	BOOST_CHECK_EQUAL(l1a->state(m,time), OWNER);
	BOOST_CHECK_EQUAL(l1b->state(m,time), SHARED);	
	BOOST_CHECK_EQUAL(l2->state(m,time),  EXCLUSIVE); 

	//Read from CPU_A (from OWNER to OWNER in L1_A and
	...
	//Write from CPU_A (from OWNER to MODIFIED in L1_A and
	...
	//Write from CPU_B (from SHARED to MODIFIED in L1_B and
	//from OWNER to INVALID in L1_A )
	...
	BOOST_CHECK_EQUAL(l1b->state(m,time), MODIFIED);	
	BOOST_CHECK_EQUAL(l2->state(m,time),  EXCLUSIVE); //WRITE BACK; no change of state

	//Write from CPU_A (from INVALID to MODIFIED in L1_A and
	//from MODIFIED to INVALID in L1_B )
	...
	BOOST_CHECK_EQUAL(lat,10u); //total write-through latency
	time+=lat+TIME_TO_STABLE;
    ...
    BOOST_TEST_MESSAGE("***_WB_ 1st Write CPU A in 4th memory location" );	
	mt.reset();
...
BOOST_AUTO_TEST_CASE( test_moesi_states_with_bus_2levels_WT_ )
{
	//CPU A - L1
	Parameters p1a;
	p1a.set("size",      "16");
	...
	//Write from CPU_B (from NOT_FOUND to MODIFIED in L1_B and
	//from MODIFIED to INVALID in L1_A)
    BOOST_TEST_MESSAGE("***_WT_ 1st Write CPU B in 4th memory location" );	
	mt.reset();
	lat=l1b->write(m4,time,mt,NOT_FOUND).latency();
	BOOST_CHECK_EQUAL(lat,10+20+100+10u); //total readx+write-through latency
	time+=lat+TIME_TO_STABLE;
