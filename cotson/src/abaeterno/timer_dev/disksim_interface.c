...
// $Id: disksim_interface.c 11 2010-01-08 13:20:58Z paolofrb $
/*
...
#undef INLINE // avoid double definition

#include <disksim/src/disksim_global.h>
#include <disksim/src/disksim_ioface.h>
#include <disksim_interface.h>
//#include <disksim/disksim_disk.h>
...
disksim_interface_initialize (const char *pfile, 
			      const char *ofile,
			      disksim_interface_complete_t completion,
...
void 
disksim_interface_shutdown (struct disksim_interface *iface, double curtime)
{
	disksim = iface->disksim;

//	fprintf (stderr, "disksim_shutdown\n");

// 	if ((curtime + 0.001) < simtime) {
...
/* totals.  "curtime" should be the current simulated time of the       */
/* system-level simulation.                                                */

void 
disksim_interface_dump_stats (struct disksim_interface *iface, double curtime)
{
	disksim = iface->disksim;

//	fprintf (stderr, "disksim_dump_stats\n");

	if ((disksim->intq) && (disksim->intq->time < curtime) && ((disksim->intq->time + 0.001) >= curtime)) 
...
/* as a slave of a system-level simulation.  "curtime" should be the    */
...
//	fprintf (stderr, "disksim_internal_event\n");
	...
	if (disksim->intq != NULL 
       		&& (disksim->intq->time + 0.001) < curtime) 
	{
		fprintf (stderr, "external time is ahead of disksim time: %f > %f\n", curtime, disksim->intq->time);
		exit (1);
	}
...
//	fprintf(stderr, "disksim_internal_event: intq->time=%f curtime=%f\n", disksim->intq->time, curtime);
	...
	while ((disksim->intq != NULL) 
				&& (disksim->intq->time <= (curtime + 0.001))) 
	{
...
//		fprintf (stderr, "handling internal event: type %d\n", disksim->intq->type);
	...
	if (disksim->intq != NULL) 
	{
		...
		iface->sched_fn(disksim_interface_internal_event, disksim->intq->time, iface->ctx);
	}
...
/* it wants to issue a request into disksim.  "curtime" should be the */
...
//	fprintf (stderr, "disksim_interface_request_arrive \n");
	disksim = iface->disksim;
	...
	new->bcount = requestdesc->bytecount / 512; //Block size assumed to by 512 bytes
...
//	fprintf (stderr, "disksim_interface_request_arrive done\n");
}
